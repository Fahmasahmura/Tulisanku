				...
				<div class="col-md-8">
						...
						if (mysqli_num_rows($exec) > 0) {
							while ($post = mysqli_fetch_assoc($exec)) {
								...
								<div class="post">
									...
									<div class="post-info">
										<p class="lead">
											Published on: <?php echo htmlentities($post_date); ?> | Category:
											<?php echo htmlentities($post_category); ?> | By: <?php echo $post_author; ?>
										</p>
									</div>
					...
					<div class="comment-section">
						<form method="POST" action="comment.php?>'">
							...
							<div class="form-group">
								<label>Comment</label>
								<textarea name="comment" placeholder="Your Comment Here" class="form-control"
									rows="10"></textarea>
							</div>
							<div class="form-group">
						...
						<div class="panel-body">
							...
							while ($recentPost = mysqli_fetch_assoc($exec)) {
								...
								<nav>
									<ul>
										<li><a
												href="Post.php?id=<?php echo $postID; ?>"><?php echo $recentPost['title'] ?></a>
										</li>
									</ul>
								</nav>
								<?php
								...
								<ul>
									...
									if (mysqli_num_rows($exec) > 0) {
										while ($category = mysqli_fetch_assoc($exec)) {
											?>
											<li><a
													href="Blog.php?category=<?php echo $category['cat_name'] ?>"><?php echo $category['cat_name'] ?></a>
											</li>
											<?php
										}
									}
